...
const api = axios.create({
  // Using type casting to prevent Vite env underline errors
...
// --- 2. Candidate API (For Applicants) ---
...
export const candidateApi = {
  ...
  downloadCertificate: (simulationId: string) =>
    api.get(`/candidate/simulations/${simulationId}/certificate`, {
      responseType: 'blob'
    }),
};

// --- 3. Employer API (For Recruiters/Companies) ---

export const employerApi = {
  createSimulation: (data: unknown) => 
    api.post('/employer/simulations', data),

  getSimulations: () => 
    api.get('/employer/simulations'),

  getSimulationDetails: (id: string) => 
    api.get(`/employer/simulations/${id}`),

  inviteCandidates: (simulationId: string, emails: string[]) => 
    api.post(`/employer/simulations/${simulationId}/invite`, { emails }),

  getCandidateResults: (simulationId: string) => 
    api.get(`/employer/simulations/${simulationId}/results`),

  getAIReport: (candidateId: string, simulationId: string) =>
    api.get(`/employer/reports/${candidateId}/${simulationId}`),
};

// --- 4. Admin API (For System Management) ---
...
export const adminApi = {
  ...
  approveSimulationTemplate: (id: string) => 
    api.post(`/admin/simulations/${id}/approve`),
